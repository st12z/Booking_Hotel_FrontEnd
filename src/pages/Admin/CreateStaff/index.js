...
import { createStaff } from "../../../service/UserService/AuthService";
    ...
    const fetchApi = async () => {
      try {
        ...
        if (resRolesAdmin.code == 200) {
          const dataRoles = resRolesAdmin.data.map((item) => ({
            label: item.name,
            value: item.id,
          }));
    ...
    const fetchApi = async () => {
      try {
        ...
        if (res.code === 201) {
          ...
          connectStomp(
            "/app/sendAmountUsers",
            `Tài khoản ${data.email} đăng kí thành công`
          );
          connectStomp("/app/sendNotification", {
            content: `Tài khoản ${data.email} đăng kí thành công`,
          });
          form.resetFields();
        } else {
          ...
          <Card title="Đăng ký" bordered={false} style={{ width: "100%" }}>
            <Form onFinish={handleSubmit} form={form}>
              <Row>
                ...
                <Col span={24} >
                  <Form.Item
                    label={<h3 style={{ color: "#0057B8" }}>Roles</h3>}
                    name="roleIds"
                    rules={rules}
                  >
                    <Select
