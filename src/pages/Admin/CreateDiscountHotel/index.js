...
function CreateDiscountHotel() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await createDiscountHotel(formData);
      console.log(res);
      if (res.code == 200) {
        openNotification(
          "topRight",
          "Tạo mới discount-hotel thành công!",
          "green"
        );
      } else {
        openNotification("topRight", "Tạo mới discount-hotel thất bại!", "red");
      }
    } catch (error) {
      console.error(error);
      openNotification("topRight", "Tạo mới discount-hotel thất bại!", "red");
...
export default CreateDiscountHotel;
