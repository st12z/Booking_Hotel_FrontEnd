...
import { getAllTripTypesByPage } from "../../../service/RoomService/TripTypeService";
import { exportTripTypes } from "../../../service/ExportService/ExportService";
...
function TripTypes() {
  ...
  const fetchApi = async (pageNo) => {
    try {
      const res = await getAllTripTypesByPage(keyword, pageNo, pageSize);
      console.log(res);
      if(res.code==200){
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <>
          <img src={record.imageIcon} style={{width:"150px"}}/>
        </>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button style={{ marginRight: "10px" }}>
          <Link to={`/admin/trip-types/edit/${record.id}`}>
            {<EditOutlined />}
          </Link>
        </Button>
      ),
  ...
  return (
    <>
      <h2>Danh sách loại điểm đến</h2>
      <Button color="green" variant="solid" style={{ marginBottom: "20px" }}>
        <Link to="/admin/trip-types/create">
          <PlusOutlined />
          Tạo mới
        </Link>
      </Button>
