...
import { getDiscountCarById, updateDiscountCars } from "../../../service/RoomService/DiscountCarsService";
const { RangePicker } = DatePicker;
function EditDiscountCars() {
  ...
  const [discountCar, setDiscountCar] = useState();
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        const res = await getDiscountCarById(id);
        console.log(res);
        if (res.code === 200) {
          setDiscountCar(res.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateDiscountCars(id, formData);
      console.log(res);
      if (res.code == 200) {
        openNotification(
          "topRight",
        ...
        openNotification(
          "topRight",
          "Cập nhật discount-car thất bại!",
          "red"
        );
      }
    } catch (error) {
      console.error(error);
      openNotification("topRight", "Cập nhật discount-car thất bại!", "red");
    } finally {
      setLoading(false);
    }
  };
...
export default EditDiscountCars;
