  ...
  const handleSave=(id)=>{
    ...
    const fetchApi=async()=>{
      ...
      if(res.code==200){
        ...
        openNotification("topRight","Phiếu giảm giá này đã được lưu.Vui lòng kiểm tra trong mục phiếu giảm giá!","red");
      }
      else{
    ...
    <>
      ...
      <div className="discount__item">
        ...
        <div className="discount__item__save">
          {saved ? (
            <Tooltip title="Đã có">
              <Button type="primary" onClick={()=>handleSave(item.id)} disabled={saved}>
                Lưu
            </Button>
            </Tooltip>
          ):(
